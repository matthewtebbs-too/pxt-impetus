...
```blocks
    ...
    world3d.scene().add(light.directional(design.colorPicker(Palette.white), 1), math.unitVector())
    boxshape = design.box(20, 20, 20)
    for (let i = 0; i < 2000; i++) {
        object = object3d.mesh(boxshape, material.ofColor(design.colorRandom()))
